...
OptionParser::OptionParser() :
		prefix("output"), no_prompt(true), verbose(false), processUU(true), max_isize(1000), big_s_bps(20), frag_size(-1), cut_sr(-1), nstdevs(3), isize_samples(1000), q(0), min_read_len(0), coverage_cutoff(0), n_cutoff(5), plotrange(1000), sd_cutoff_disc(3), sd_cutoff_cl(-1), support_mps(-1), support_mpf(-1), min_mapq(
				0), alt_map_max(10000), alt_map_max_clip(numeric_limits<int32_t>::max()), sv_size_cutoff(1000000000), support_reads(1), te_size_max(100000), del_ins_size_cutoff_d(0.8), del_ins_size_cutoff_u(1.2), ovl(0.8), filter_dups(false), filter_dups_by_flag(false), remove_dup(false), clip(false), ad_align(true), use_all_align(false), include_other(false), generate_mapped_sc_um_file(false), is_cleaning(false) {
	program_names.insert("tea");
	program_names.insert("tea_v");
	program_names.insert("tea_u");
	program_names.insert("transduction");
	program_names.insert("orphan");
	program_names.insert("post");
...
OptionParser::OptionParser(int argc, char **argv) :
		prefix("-"), no_prompt(true), verbose(false), processUU(true), max_isize(1000), big_s_bps(20), frag_size(-1), cut_sr(-1), nstdevs(3), isize_samples(1000), q(0), min_read_len(0), coverage_cutoff(0), n_cutoff(5), plotrange(1000), sd_cutoff_disc(3), sd_cutoff_cl(-1), support_mps(-1), support_mpf(-1), min_mapq(
				0), alt_map_max(100), alt_map_max_clip(numeric_limits<int32_t>::max()), sv_size_cutoff(1000000000), support_reads(1), te_size_max(100000), del_ins_size_cutoff_d(0.8), del_ins_size_cutoff_u(1.2), ovl(0.8), filter_dups(false), filter_dups_by_flag(false), remove_dup(false), clip(false), ad_align(true), use_all_align(false), include_other(false), generate_mapped_sc_um_file(false), is_cleaning(false) {
	TimeChecker checker;
	n_cores = checker.get_number_of_cores();
	tmp_directory = "/tmp";
	string line;
